...
import random
from dataset.acdc_v2 import ACDC
...
def acdc_v2_loader(config, verbose: bool = True) -> dict:
    ...
    if verbose:
        print("ACDC 3D:")
        for mode in modes:
            print(f"├──> Length of {mode}_dataset: {len(datasets[mode])} and {mode}_patients: {len(loaders[mode])//2}")
