...
class Brats2018(Dataset):
    ...
    def __getitem__(self, index):
        ...
        patient = self.patients_list[index]
    ...
    def _load_data(self, patient, modalities):
        volumes = []
        for modality in modalities:
            patient_id = os.path.split(patient)[-1]
            volume_path = os.path.join(patient, patient_id + "_" + modality + ".nii.gz")
    ...
    def split_dataset(self, patients_dir):
        ...
        if self.mode == "tr":
            self.patients_list = patients_dir[te_index + vl_index : (int)(len(patients_dir) - 150)]
        elif self.mode == "te":
...
if __name__ == "__main__":
    ...
    dataset = Brats2018(data_root, crop_size=crop_size, mode="tr")
