...
def replace_inf(x):
    if isinstance(x, (list, np.ndarray)):  # check if x is a list or a numpy array
        return [374 if item == np.inf else item for item in x]
    else:
        return 374 if x == np.inf else x


def get_LesionWiseResults(pred_file, gt_file, challenge_name="BraTS-MET", output=None):
    """
    Computes the Lesion-wise scores for pair of prediction and ground truth
    segmentations
    ...
    for l in range(len(label_values)):
        ...
        for col in metric_df.columns:
            metric_df[col] = metric_df[col].apply(replace_inf)
