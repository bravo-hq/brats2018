...
class Multiresblock(nn.Module):
    def __init__(
        self, input_features: int, corresponding_unet_filters: int, alpha: float = 1.67
    ) -> None:
        """
        MultiResblock
        Arguments:
        ...
        self.batch_norm1 = nn.BatchNorm2d(
            int(self.W * 0.5) + int(self.W * 0.167) + int(self.W * 0.333), affine=False
        )
...
class MultiResUnet(nn.Module):
    def __init__(self, channels: int, filters: int = 32, nclasses: int = 1) -> None:
        ...
        self.multiresblock1 = Multiresblock(
            input_features=channels, corresponding_unet_filters=self.filters
        )
        ...
        self.respath1 = Respath(
            input_features=self.in_filters1, filters=self.filters, respath_length=4
        )
        self.multiresblock2 = Multiresblock(
            input_features=self.in_filters1, corresponding_unet_filters=self.filters * 2
        )
        ...
        self.respath2 = Respath(
            input_features=self.in_filters2, filters=self.filters * 2, respath_length=3
        )
        self.multiresblock3 = Multiresblock(
            input_features=self.in_filters2, corresponding_unet_filters=self.filters * 4
        )
        ...
        self.respath3 = Respath(
            input_features=self.in_filters3, filters=self.filters * 4, respath_length=2
        )
        self.multiresblock4 = Multiresblock(
            input_features=self.in_filters3, corresponding_unet_filters=self.filters * 8
        )
        ...
        self.respath4 = Respath(
            input_features=self.in_filters4, filters=self.filters * 8, respath_length=1
        )
        ...
        self.multiresblock8 = Multiresblock(
            input_features=self.concat_filters3,
        ...
        self.multiresblock9 = Multiresblock(
            input_features=self.concat_filters4, corresponding_unet_filters=self.filters
        )
        self.in_filters9 = (
            int(self.filters * self.alpha * 0.5)
            + int(self.filters * self.alpha * 0.167)
        ...
        self.conv_final = Conv2d_batchnorm(
            input_features=self.in_filters9,
