...
class DataAugmentationTransform(object):
    ...
    def get_aug_policy_1(self):
        dns_1 = A.Compose(
            [
                ...
                A.Cutout(
                    max_h_size=int(self.input_size[0] * 0.2),
                    max_w_size=int(self.input_size[1] * 0.2),
                    num_holes=1,
                    p=0.7,
                ),
                A.Normalize(),
        ...
        dns_1 = A.Compose(
            [
                ...
                A.OneOf(
                    [
                        A.OpticalDistortion(distort_limit=1.0),
                        A.GridDistortion(num_steps=3, distort_limit=1.0),
                        A.ElasticTransform(alpha=3),
                        # A.ElasticTransform(p=1, alpha=120, sigma=120 * 0.05, alpha_affine=120 * 0.03)
                    ],
                    p=0.7,
                ...
                A.Resize(self.input_size[0], self.input_size[1]),
                A.Cutout(
                    max_h_size=int(self.input_size[0] * 0.1),
                    max_w_size=int(self.input_size[1] * 0.1),
                    num_holes=3,
                    p=0.7,
                ),
                A.Normalize(),
            ...
            [
                ...
                A.OneOf(
                    [
                        ...
                        A.GridDistortion(num_steps=1, distort_limit=1.0),
                ...
                A.OneOf(
                    [
                        A.Cutout(
                            max_h_size=int(self.input_size[0] * 0.375),
                            max_w_size=int(self.input_size[1] * 0.375),
                            num_holes=1,
                            p=0.7,
                        ),
                        A.Cutout(
                            max_h_size=int(self.input_size[0] * 0.200),
                            max_w_size=int(self.input_size[1] * 0.200),
                            num_holes=2,
                            p=0.7,
                        ),
                        A.Cutout(
                            max_h_size=int(self.input_size[0] * 0.100),
                            max_w_size=int(self.input_size[1] * 0.100),
                            num_holes=3,
                            p=0.7,
                        ),
                    ],
    ...
    def get_pixel_level_transform(self, config, img_path_list=None):
        pixel_transform = A.Compose(
            [
                globals()[t](**p)
                for t, p in config["levels"]["pixel"]["transforms"].items()
            ]
        )
        return pixel_transform

    def get_spacial_level_transform(self, config, img_path_list=None):
        spacial_transform = A.Compose(
            [
                globals()[t](**p)
                for t, p in config["levels"]["spacial"]["transforms"].items()
            ]
        )
    ...
    def get_color_transform(self):
        color_transform = T.ColorJitter(
            brightness=0.3, hue=0.05, saturation=0.1, contrast=0.3
        )
    ...
    def get_other_transform(self):
        ...
        other_transform = T.RandomChoice([T.RandomApply([pad_transform], p=0.8)])
