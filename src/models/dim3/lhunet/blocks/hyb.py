...
class BaseHybridBlock():
    def combine(self, tensors: list[torch.Tensor]):
        ...
        #     return res
            ...
            raise NotImplementedError(f"Not implemented combining mode for <{self.res}>")
...
class HybridEncoder(BaseBlock, BaseHybridBlock):
    def __init__(
        ...
        **kwds: Any,
        ...
        self.downs, self.vits, self.convs = nn.ModuleList(), nn.ModuleList(), nn.ModuleList()
        infos = zip(
            cnn_blocks, vit_blocks, io_channles,
            cnn_kernel_sizes, cnn_strides, cnn_maxpools, cnn_dropouts, 
            vit_input_sizes, vit_proj_sizes, vit_repeats, vit_num_heads, vit_dropouts,
        )
        for c_blkc, t_blkc, (ich, och), \
            c_ks, c_st, c_mp, c_do, \
            t_is, t_ps, t_rp, t_nh, t_do in infos:
            
            self.downs.append(
                nn.Sequential(
                    get_cnn_block(code=c_blkc)(
                        spatial_dims=spatial_dims,
                        in_channels=ich, out_channels=och,
                        kernel_size=c_ks,
            ...
            self.vits.append(
                nn.Sequential(*[get_vit_block(code=t_blkc)(
                    input_size=t_is,
                    hidden_size=och,
                    proj_size=t_ps,
                    num_heads=t_nh,
...
class HybridDecoder(BaseBlock, BaseHybridBlock):
    def __init__(self,
        ...
        **kwds: Any,
        ...
        # >>> checking
        if len(cnn_blocks)==1: cnn_blocks *= len(cnn_kernel_sizes)
        if len(vit_blocks)==1: vit_blocks *= len(vit_input_sizes)
        ...
        assert (len(cnn_blocks) == len(cnn_kernel_sizes) == len(features)
            ), "cnn_blocks, cnn_kernel_sizes, and features must have the same length"
        if not isinstance(cnn_dropouts, list): cnn_dropouts = [cnn_dropouts for _ in features]
        if not isinstance(vit_dropouts, list): vit_dropouts = [vit_dropouts for _ in features]
        assert (len(cnn_kernel_sizes) == len(tcv_strides) == len(features)
        ...
        self.ups , self.convs, self.vits, self.convs_o = [nn.ModuleList() for _ in range(4)]
        info = zip(cnn_blocks, vit_blocks, io_channles, skip_channels,
            tcv_kernel_sizes, tcv_strides,
            cnn_kernel_sizes, cnn_dropouts,
            vit_input_sizes, vit_proj_sizes, vit_repeats, vit_num_heads, vit_dropouts
        )
        
        for c_blkc, t_blkc, (ich, och), skch, \
            tcv_ks, tcv_st, \
            c_ks, c_do, \
            t_is, t_ps, t_rp, t_nh, t_do in info:
            ...
            self.vits.append(
                nn.Sequential(*[get_vit_block(code=t_blkc)(
                    input_size=t_is,
                    hidden_size=och if self.arch_mode in ["sequential-lite", "collective"] else vit_chs,
                    proj_size=t_ps,
                    num_heads=t_nh,
                    dropout_rate=t_do,
                    pos_embed=True,
            ...
            self.convs.append(
                get_cnn_block(code=c_blkc)(
                    ...
                    norm_name=norm_name,
                    act_name=act_name,
                ) if self.arch_mode=="parallel" else nn.Identity()
            )
