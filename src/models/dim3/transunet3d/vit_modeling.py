...
class Block(nn.Module):
    def __init__(self, config, vis, use_layer_scale):
        ...
        self.ls1 = (
            LayerScale(config.hidden_size, init_values=1e-5)
            if use_layer_scale
            else nn.Identity()
        )
        self.ls2 = (
...
class Transformer(nn.Module):
    def __init__(self, config, feat_size, vis, feat_channels, use_layer_scale):
        ...
        self.embeddings = Embeddings(
            config, feat_size=feat_size, in_channels=feat_channels
        )
    ...
    def forward(self, input_ids):
        ...
        (
            ...
            n_patch,
            hidden,
        ) = encoded.size()  # reshape from (B, n_patch, hidden) to (B, h, w, hidden)
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            if posemb.size() == posemb_new.size():
                ...
                posemb_up = nn.Upsample(size=ntok_new)(
                    posemb_grid.permute(1, 0).unsqueeze(0)
                )
