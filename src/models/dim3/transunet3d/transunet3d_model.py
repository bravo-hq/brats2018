...
class Generic_TransUNet_max_ppbp(SegmentationNetwork):
    ...
    def __init__(
        ...
        convolutional_upsampling=True,  # TODO default False
        ...
        # self.inference_apply_nonlin = nn.Sigmoid() # softmax_helper
        # newly added
        self.is_fam = is_fam
        (
            self.is_max,
        ...
        if self.is_max:
            ...
            cfg = {
                ...
                else num_queries,  # N=K if 'fixed matching', else default=100,
                "nheads": 8,
            ...
            input_proj_list = (
                []
            )  # from low resolution to high resolution (res4 -> res1), [1, 1024, 14, 14], [1, 512, 28, 28], 1, 256, 56, 56], [1, 64, 112, 112]
            ...
            if self.is_max_ms:  # use multi-scale feature as Transformer decoder input
                if self.is_max_ms_fpn:
                    for idx, in_channels in enumerate(
                        decoder_channels[:max_n_fpn]
                    ):  # max_n_fpn=4: 1/32, 1/16, 1/8, 1/4
                    ...
                    self.linear_encoder_feature = nn.Conv3d(
                        max_hidden_dim * max_n_fpn, max_hidden_dim, 1, 1
                    )  # concat four-level feature
                else:
                    for idx, in_channels in enumerate(
                        [decoder_channels[i] for i in self.max_ms_idxs]
                    ):
                        input_proj_list.append(
                            nn.Sequential(
                                nn.Conv3d(in_channels, max_hidden_dim, kernel_size=1),
                ...
                self.linear_mask_features = nn.Conv3d(
                    ...
                    stride=1,
                    padding=0,
                )  # following SingleScale, high-level feat, obtain seg_map
            else:
                self.linear_encoder_feature = nn.Conv3d(
                ...
                self.linear_mask_features = nn.Conv3d(
                    ...
                    stride=1,
                    padding=0,
                )  # low-level feat, dot product Trans-feat
            ...
            if self.is_masked_attn:
                ...
                cfg["num_feature_levels"] = (
                    1 if not self.is_max_ms or self.is_max_ms_fpn else 3
                )
                ...
                cfg["dropout"], cfg["enc_layers"], cfg["deep_supervision"] = (
                    0.1,
                    0,
                    False,
                )
    ...
    def forward(self, x):
        ...
        if self.is_max:
            if self.is_max_ms:  # is_max_ms_fpn
                ...
                mask_features = self.linear_mask_features(
                    ds_feats[-1]
                )  # following SingleScale
            else:
            ...
            predictions = self.predictor(
                transformer_decoder_in_feature, mask_features, mask=None
            )
            ...
            if self.is_max_cls and self.is_max_ds:
                ...
                aux_out = [
                    p["pred_masks"] for p in predictions["aux_outputs"]
                ]  # ascending order
...
class HungarianMatcher3D(nn.Module):
    ...
    def compute_ce_loss(self, inputs, targets):
        ...
        loss = F.binary_cross_entropy_with_logits(
            inputs.flatten(1), targets.flatten(1), reduction="none"
        )
    ...
    def memory_efficient_forward(self, outputs, targets):
        ...
        for b in range(bs):
            ...
            tgt_mask = targets[b]["masks"].to(
                out_mask
            )  # [K, D, H, W], K is number of classes shown in this image, and K < n_class
...
def compute_loss_hungarian(
    ...
    src_idx = matcher._get_src_permutation_idx(
        indices
    )  # return a tuple of (batch_idx, src_idx)
    tgt_idx = matcher._get_tgt_permutation_idx(
        indices
    )  # return a tuple of (batch_idx, tgt_idx)
    assert len(tgt_idx[0]) == sum(
        [len(t["masks"]) for t in targets]
    )  # verify that all masks of (K1, K2, ..) are used

    # step2 : compute mask loss
    src_masks = outputs["pred_masks"]
    src_masks = src_masks[src_idx]  # [len(src_idx[0]), D, H, W] -> (K1+K2+..., D, H, W)
    target_masks = torch.cat(
        [t["masks"] for t in targets], dim=0
    )  # (K1+K2+..., D, H, W) actually
...
def get_uncertain_point_coords_with_randomness(
    ...
    point_coords = torch.rand(
        num_boxes, num_sampled, n_dim, device=coarse_logits.device
    )  # (K, 37632, 3); uniform dist [0, 1)
    point_logits = point_sample_3d(
    ...
    if num_random_points > 0:
        ...
        point_coords = torch.cat(
            [
                point_coords,
                torch.rand(
                    num_boxes, num_random_points, n_dim, device=coarse_logits.device
                ),
            ],
            dim=1,
        )  # [K, 12544, 3]
