...
class SwinTransformerBlock_kv(nn.Module):
    ...
    def forward(self, x, mask_matrix, skip=None, x_up=None):
        ...
        skip = F.pad(
            skip, (0, 0, 0, pad_r, 0, pad_b, 0, pad_g)
        )  # pad�ĵ�һ�������Ӧ���һ��ά�ȣ����һ�������Ӧ��һ��ά��
        _, Sp, Hp, Wp, _ = skip.shape

        x_up = F.pad(
            x_up, (0, 0, 0, pad_r, 0, pad_b, 0, pad_g)
        )  # pad�ĵ�һ�������Ӧ���һ��ά�ȣ����һ�������Ӧ��һ��ά��
        # cyclic shift
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm, tag=None):
        ...
        if tag == 0:
            self.reduction = nn.Conv3d(
                dim, dim * 2, kernel_size=[1, 3, 3], stride=[1, 2, 2], padding=[0, 1, 1]
            )
        elif tag == 1:
            self.reduction = nn.Conv3d(
                dim, dim * 2, kernel_size=[3, 3, 3], stride=[2, 2, 2], padding=[1, 1, 1]
            )
        else:
            self.reduction = nn.Conv3d(
                dim, dim * 2, kernel_size=[3, 3, 3], stride=[2, 2, 2], padding=[0, 1, 1]
            )
