...
def convert_id_to_task_name(task_id: int):
    ...
    if nnFormer_cropped_data is not None:
        candidates_cropped = subdirs(
            nnFormer_cropped_data, prefix=startswith, join=False
        )
    else:
        candidates_cropped = []
    ...
    if len(unique_candidates) == 0:
        raise RuntimeError(
            ...
            % (
                task_id,
                os.environ.get("unetr_pp_preprocessed")
                if os.environ.get("unetr_pp_preprocessed") is not None
                else "None",
                os.environ.get("RESULTS_FOLDER")
                if os.environ.get("RESULTS_FOLDER") is not None
                else "None",
                os.environ.get("unetr_pp_raw_data_base")
                if os.environ.get("unetr_pp_raw_data_base") is not None
                else "None",
            )
        )
